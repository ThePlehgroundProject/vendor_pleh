...
func init() {
	android.RegisterModuleType("pleh_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = plehExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = plehExpandVariables(ctx, dep_file)
	...
	cmd := plehExpandVariables(ctx, String(g.properties.Cmd))
